...
func (p Property) GoFieldName() string {
	...
	if extension, ok := p.Extensions[internal.ExtGoName]; ok {
	...
	if GlobalState.options.Compatibility.AllowUnexportedStructFieldNames {
		if extension, ok := p.Extensions[internal.ExtOapiCodegenOnlyHonourGoName]; ok {
...
func GenerateGoSchema(sref *openapi3.SchemaRef, path []string) (Schema, error) {
	...
	if extension, ok := schema.Extensions[internal.ExtPropGoType]; ok {
		typeName, err := extTypeName(extension)
		if err != nil {
			return outSchema, fmt.Errorf("invalid value for %q: %w", internal.ExtPropGoType, err)
		}
	...
	if extension, ok := schema.Extensions[internal.ExtPropGoTypeSkipOptionalPointer]; ok {
		skipOptionalPointer, err := extParsePropGoTypeSkipOptionalPointer(extension)
		if err != nil {
			return outSchema, fmt.Errorf("invalid value for %q: %w", internal.ExtPropGoTypeSkipOptionalPointer, err)
		}
	...
	if t.Slice() == nil || t.Is("object") {
		...
		// of this schema.
		if extension, ok := schema.Extensions[internal.ExtGoTypeName]; ok {
			typeName, err := extTypeName(extension)
			if err != nil {
				return outSchema, fmt.Errorf("invalid value for %q: %w", internal.ExtGoTypeName, err)
			}
		...
		for _, key := range []string{internal.ExtEnumVarNames, internal.ExtEnumNames} {
...
func GenFieldsFromProperties(props []Property) []string {
	...
	for i, p := range props {
		...
		if p.Deprecated {
			...
			if extension, ok := p.Extensions[internal.ExtDeprecationReason]; ok {
		...
		// Check x-go-type-skip-optional-pointer, which will override if the type
		// should be a pointer or not when the field is optional.
		if extension, ok := p.Extensions[internal.ExtPropGoTypeSkipOptionalPointer]; ok {
		...
		// Support x-omitempty
		if extOmitEmptyValue, ok := p.Extensions[internal.ExtPropOmitEmpty]; ok {
		...
		if extension, ok := p.Extensions[internal.ExtPropGoJsonIgnore]; ok {
		...
		if extension, ok := p.Extensions[internal.ExtPropExtraTags]; ok {
