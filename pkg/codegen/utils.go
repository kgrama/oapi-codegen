...
func refPathToGoType(refPath string, local bool) (string, error) {
	...
	goPkg, ok := globalstate.GlobalState.ImportMapping[remoteComponent]
	...
	if goPkg.Path == globalstate.ImportMappingCurrentPackage {
...
func refPathToGoTypeSelf(refPath string, local bool) (string, error) {
	...
	// the spec.
	name, err := findSchemaNameByRefPath(refPath, globalstate.GlobalState.Spec)
	if err != nil {
...
func refPathToGoTypeRemote(flatComponent string, goPkg globalstate.GoImport) (string, error) {
...
func ParseGoImportExtension(v *openapi3.SchemaRef) (*globalstate.GoImport, error) {
...
func MergeImports(dst, src map[string]globalstate.GoImport) {
	for k, v := range src {
		dst[k] = v
	}
}
