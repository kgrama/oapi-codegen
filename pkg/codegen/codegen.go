...
func Generate(spec *openapi3.T, opts Configuration) (string, error) {
	...
	globalstate.GlobalState.Options = opts
	globalstate.GlobalState.Spec = spec
	globalstate.GlobalState.ImportMapping = globalstate.ConstructImportMapping(opts.ImportMapping)
	...
	if globalstate.GlobalState.Options.OutputOptions.ClientTypeName == "" {
		globalstate.GlobalState.Options.OutputOptions.ClientTypeName = defaultClientTypeName
	}
	...
	TemplateFunctions["opts"] = func() Configuration { return globalstate.GlobalState.Options }
	...
	if opts.Generate.EmbeddedSpec {
		inlinedSpec, err = GenerateInlinedSpec(t, globalstate.GlobalState.ImportMapping, spec)
		if err != nil {
			return "", fmt.Errorf("error generating Go handlers for Paths: %w", err)
		}
	}
	...
	externalImports := append(globalstate.GlobalState.ImportMapping.GoImports(), globalstate.ImportMap(xGoTypeImports).GoImports()...)
	...
	for _, tp := range types {
		...
		if len(tp.Schema.EnumValues) > 0 {
			...
			enums = append(enums, EnumDefinition{
				...
				PrefixTypeName: globalstate.GlobalState.Options.Compatibility.AlwaysPrefixEnumValues,
			})
		}
	}
...
func GenerateImports(t *template.Template, externalImports []string, packageName string, versionOverride *string) (string, error) {
	...
	context := struct {
		...
		AdditionalImports: globalstate.GlobalState.Options.AdditionalImports,
	}
...
func OperationSchemaImports(s *Schema) (map[string]globalstate.GoImport, error) {
	res := map[string]globalstate.GoImport{}
...
func OperationImports(ops []OperationDefinition) (map[string]globalstate.GoImport, error) {
	res := map[string]globalstate.GoImport{}
	for _, op := range ops {
...
func GetTypeDefinitionsImports(swagger *openapi3.T, excludeSchemas []string) (map[string]globalstate.GoImport, error) {
	res := map[string]globalstate.GoImport{}
	if swagger.Components == nil {
		return res, nil
	}
	...
	for _, imprts := range []map[string]globalstate.GoImport{schemaImports, reqBodiesImports, responsesImports, parametersImports} {
		MergeImports(res, imprts)
	}
	return res, nil
}

func GoSchemaImports(schemas ...*openapi3.SchemaRef) (map[string]globalstate.GoImport, error) {
	res := map[string]globalstate.GoImport{}
...
func GetSchemaImports(schemas map[string]*openapi3.SchemaRef, excludeSchemas []string) (map[string]globalstate.GoImport, error) {
	res := map[string]globalstate.GoImport{}
	excludeSchemasMap := make(map[string]bool)
	for _, schema := range excludeSchemas {
		excludeSchemasMap[schema] = true
...
func GetRequestBodiesImports(bodies map[string]*openapi3.RequestBodyRef) (map[string]globalstate.GoImport, error) {
	res := map[string]globalstate.GoImport{}
...
func GetResponsesImports(responses map[string]*openapi3.ResponseRef) (map[string]globalstate.GoImport, error) {
	res := map[string]globalstate.GoImport{}
...
func GetParametersImports(params map[string]*openapi3.ParameterRef) (map[string]globalstate.GoImport, error) {
	res := map[string]globalstate.GoImport{}
